  ...
  app_settings = merge(
    {
      WEBSITES_PORT   = "8000"
      DEPLOYMENT_TYPE = var.deployment_type
      ALLOWED_HOSTS   = "app-${var.project}-${var.deployment_type}-${var.environment}-staging.azurewebsites.net,app-${var.project}-${var.deployment_type}-${var.environment}.azurewebsites.net"
      SECRET_KEY      = var.key_vault_name != "" ? "@Microsoft.KeyVault(VaultName=${var.key_vault_name};SecretName=django-secret-key)" : var.django_secret_key
    },
    var.db_host != "" ? {
      DB_HOST     = var.db_host
